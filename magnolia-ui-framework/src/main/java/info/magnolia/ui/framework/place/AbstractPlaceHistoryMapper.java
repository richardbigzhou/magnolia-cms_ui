...
public abstract class AbstractPlaceHistoryMapper implements PlaceHistoryMapper {
    ...
    public Place getPlace(String token) {
        ...
        if (colonAt > 0) {
            String prefix = token.substring(0, colonAt);
            String remainder = token.substring(colonAt + 1);
            PlaceTokenizer<?> tokenizer = getTokenizer(prefix);
            if (tokenizer != null) {
                return tokenizer.getPlace(remainder);
            }
        }
        return null;
    }
     ...
     * @return the prefix and token, or null
     */
