...
public class AboutAppBaseApp extends BaseApp {
    public AboutAppBaseApp(AppContext appContext, AppView view) {
    ...
    public void start(final Location location) {
        ...
        final String appName = appContext.getAppDescriptor().getName();

        List<SubAppDescriptor> subApps = new ArrayList<>(appContext.getAppDescriptor().getSubApps().values());
        Collections.reverse(subApps);
        for (SubAppDescriptor subAppDescriptor : subApps) {
