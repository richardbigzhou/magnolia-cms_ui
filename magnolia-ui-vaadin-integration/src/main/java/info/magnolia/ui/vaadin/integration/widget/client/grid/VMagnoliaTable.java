 ...
 *
 */
package info.magnolia.ui.vaadin.integration.widget.client.grid;


import info.magnolia.ui.vaadin.integration.widget.client.grid.VMagnoliaTable.VScrollTableBody.VScrollTableRow;
...
public class VMagnoliaTable extends FlowPanel implements Table, ScrollHandler, VHasDropHandler, FocusHandler, BlurHandler, Focusable, ActionOwner {
    ...
    private class SelectionRange {
        ...
        private boolean inRange(VScrollTableRow row) {
            return row.getIndex() >= startRow.getIndex() && row.getIndex() < startRow.getIndex() + length;
        }
        ...
        public Collection<SelectionRange> split(VScrollTableRow row) {
            ...
            if (!(getEndIndex() - startOfSecondRange < 0)) {
                ...
                VScrollTableRow startOfRange = scrollBody.getRowByRowIndex(startOfSecondRange);
        ...
        if (partialRowUpdates != null || partialRowAdditions != null) {
            ...
            if (rowData != null) {
                ...
                if (!recalcWidths && initializedAndAttached) {
                    ...
                    } else if (!isScrollPositionVisible() || totalRowsChanged || lastRenderedHeight != scrollBody.getOffsetHeight()) {
    ...
    private void updateFirstVisibleAndScrollIfNeeded(UIDL uidl) {
        ...
        if (firstvisible != lastRequestedFirstvisible && scrollBody != null) {
            ...
            scrollBodyPanel.setScrollPosition(measureRowHeightOffset(firstvisible));
        }
    }
    ...
    private void purgeUnregistryBag() {
        for (Iterator<Panel> iterator = lazyUnregistryBag.iterator(); iterator.hasNext();) {
    ...
    protected void discardRowsOutsideCacheWindow() {
        ...
        if (serverCacheFirst != -1) {
            ...
            if (firstRowToKeep < scrollBody.getFirstRendered() || lastRowToKeep > scrollBody.getLastRendered()) {
                debug("*** Server wants us to keep " + serverCacheFirst + "-" + serverCacheLast + " but we only have rows " + scrollBody.getFirstRendered()
    ...
    protected void addAndRemoveRows(UIDL partialRowAdditions) {
        ...
        if (partialRowAdditions.hasAttribute("hide")) {
            ...
            if (partialRowAdditions.hasAttribute("delbelow")) {
                scrollBody.insertRowsDeleteBelow(partialRowAdditions, partialRowAdditions.getIntAttribute("firstprowix"),
                        partialRowAdditions.getIntAttribute("numprows"));
            } else {
                scrollBody.insertAndReindexRows(partialRowAdditions, partialRowAdditions.getIntAttribute("firstprowix"),
                        partialRowAdditions.getIntAttribute("numprows"));
            }
        }
    ...
    private boolean isMultiSelectModeSimple() {
        return selectMode == Table.SELECT_MODE_MULTI && multiselectmode == MULTISELECT_MODE_SIMPLE;
    }
    ...
    private boolean isMultiSelectModeDefault() {
        return selectMode == Table.SELECT_MODE_MULTI && multiselectmode == MULTISELECT_MODE_DEFAULT;
    }
        ...
        public void deferRowFetch(int msec) {
            ...
            if (reqRows > 0 && reqFirstRow < totalRows) {
                ...
                if (totalRows > pageLength
                        && ((firstRowInViewPort + pageLength > scrollBody.getLastRendered()) || (firstRowInViewPort < scrollBody.getFirstRendered()))) {
    ...
    public class HeaderCell extends Widget {
        ...
        public HeaderCell(String colId, String headerText) {
            ...
            DOM.setElementProperty(sortIndicator, "className", CLASSNAME + "-sort-indicator");
            ...
            DOM.sinkEvents(td, Event.MOUSEEVENTS | Event.ONDBLCLICK | Event.ONCONTEXTMENU | Event.TOUCHEVENTS);
        ...
        public void onBrowserEvent(Event event) {
            if (enabled && event != null) {
                if (isResizing || event.getEventTarget().cast() == colResizeWidget) {
                    ...
                    if (event.getTypeInt() == Event.ONCONTEXTMENU && !client.hasEventListeners(VMagnoliaTable.this, HEADER_CLICK_EVENT_ID)) {
        ...
        private void fireHeaderClickedEvent(Event event) {
            if (client.hasEventListeners(VMagnoliaTable.this, HEADER_CLICK_EVENT_ID)) {
        ...
        public int getNaturalColumnWidth(int columnIndex) {
            if (isDefinedWidth()) {
                ...
                if (naturalWidth < 0) {
                    ...
                    int hw = captionContainer.getOffsetWidth() + scrollBody.getCellExtraWidth();
    ...
    public class TableHead extends Panel implements ActionOwner {
        ...
        public void resizeCaptionContainer(HeaderCell cell) {
            ...
            if (cell == lastcell && columnSelector.getOffsetWidth() > 0 && columnTotalWidth >= div.getOffsetWidth() - columnSelector.getOffsetWidth()
        ...
        private void focusSlot(int index) {
            removeSlotFocus();
            if (index > 0) {
                DOM.setElementProperty(DOM.getFirstChild(DOM.getChild(tr, index - 1)), "className", CLASSNAME + "-resizer " + CLASSNAME + "-focus-slot-right");
            } else {
                DOM.setElementProperty(DOM.getFirstChild(DOM.getChild(tr, index)), "className", CLASSNAME + "-resizer " + CLASSNAME + "-focus-slot-left");
            }
            focusedSlot = index;
        }
        ...
        private void removeSlotFocus() {
            ...
            if (focusedSlot == 0) {
                DOM.setElementProperty(DOM.getFirstChild(DOM.getChild(tr, focusedSlot)), "className", CLASSNAME + "-resizer");
            } else if (focusedSlot > 0) {
                DOM.setElementProperty(DOM.getFirstChild(DOM.getChild(tr, focusedSlot - 1)), "className", CLASSNAME + "-resizer");
            }
            focusedSlot = -1;
        }
        ...
        class VisibleColumnAction extends Action {
            ...
            public VisibleColumnAction(String colKey) {
                super(VMagnoliaTable.TableHead.this);
                this.colKey = colKey;
    ...
    public class FooterCell extends Widget {
        ...
        public FooterCell(String colId, String headerText) {
            ...
            DOM.setElementProperty(captionContainer, "className", CLASSNAME + "-footer-container");
        ...
        public void setWidth(int w, boolean ensureDefinedWidth) {
            ...
            if (w == -1) {
                ...
                if (w - borderWidths >= 0) {
                    captionContainer.getStyle().setPropertyPx("width", w - borderWidths);
                } else {
                ...
                if (scrollBody != null) {
                     ...
                     */
                    int tdWidth = width + scrollBody.getCellExtraWidth() - borderWidths;
                    setWidth(tdWidth + "px");
                } else {
                    Scheduler.get().scheduleDeferred(new Command() {
                        @Override
        ...
        public void onBrowserEvent(Event event) {
            if (enabled && event != null) {
                ...
                boolean stopPropagation = true;
                if (event.getTypeInt() == Event.ONCONTEXTMENU && !client.hasEventListeners(VMagnoliaTable.this, FOOTER_CLICK_EVENT_ID)) {
        ...
        private void fireFooterClickedEvent(Event event) {
            if (client.hasEventListeners(VMagnoliaTable.this, FOOTER_CLICK_EVENT_ID)) {
                MouseEventDetails details = new MouseEventDetails(event);
        ...
        public int getNaturalColumnWidth(int columnIndex) {
            if (isDefinedWidth()) {
                ...
                if (naturalWidth < 0) {
                    ...
                    final int hw = ((Element) getElement().getLastChild()).getOffsetWidth() + scrollBody.getCellExtraWidth();
                    if (columnIndex < 0) {
                        columnIndex = 0;
                        for (Iterator<Widget> it = tHead.iterator(); it.hasNext(); columnIndex++) {
                            if (it.next() == this) {
    ...
    public class TableFooter extends Panel {
        ...
        public TableFooter() {
            ...
            DOM.setStyleAttribute(hTableWrapper, "overflow", "hidden");
            DOM.setElementProperty(hTableWrapper, "className", CLASSNAME + "-footer");
            ...
            availableCells.put(ROW_HEADER_COLUMN_KEY, new RowHeadersFooterCell());
        }
    ...
    public class VScrollTableBody extends com.google.gwt.user.client.ui.Panel {
        ...
        public int getRequiredHeight() {
            return preSpacer.getOffsetHeight() + postSpacer.getOffsetHeight() + Util.getRequiredHeight(table);
        }
        ...
        private void addRowBeforeFirstRendered(VScrollTableRow row) {
            ...
            tBodyElement.insertBefore(row.getElement(), tBodyElement.getFirstChild());
            adopt(row);
        ...
        private void unlinkRowAtActualIndex(int index) {
            final VScrollTableRow toBeRemoved = (VScrollTableRow) renderedRows.get(index);
            // Unregister row tooltip
            client.registerTooltip(VMagnoliaTable.this, toBeRemoved.getElement(), null);
            for (int i = 0; i < toBeRemoved.getElement().getChildCount(); i++) {
                ...
                client.registerTooltip(VMagnoliaTable.this, td, null);
            }
        ...
        private void setContainerHeight() {
            fixSpacers();
            DOM.setStyleAttribute(container, "height", measureRowHeightOffset(totalRows) + "px");
        }
        ...
        private void fixSpacers() {
            ...
            int postpx = measureRowHeightOffset(totalRows - 1) - measureRowHeightOffset(lastRendered);
            if (postpx < 0) {
                postpx = 0;
            }
        ...
        private ValueChangeHandler<Boolean> selectionCheckBoxValueChangeHandler = new ValueChangeHandler<Boolean>() {
        ...
        public class VScrollTableRow extends Panel implements ActionOwner, Container {
            ...
            public VScrollTableRow(UIDL uidl, char[] aligns) {
                ...
                if (rowDescription != null && !rowDescription.equals("")) {
                    ...
                    client.registerTooltip(VMagnoliaTable.this, rowElement, info);
                } else {
                    // Remove possibly previously set tooltip
                    client.registerTooltip(VMagnoliaTable.this, rowElement, null);
                }
            ...
            protected void initCellWidths() {
                ...
                for (int i = 0; i < cells; i++) {
                    int w = VMagnoliaTable.this.getColWidth(getColKeyByIndex(i));
                    if (w < 0) {
                        w = 0;
                    }
                    setCellWidth(i, w);
            ...
            /**
             * Fork changes in method: instead of referring the child by index
             * cellIdx we here access cellIx + 1, cause the 0 child is checkbox.
             * Also static checkbox width is calculated if needed and subtracted
             * from the width of the first "real" cell.
             */
            ...
            protected void initCellWithText(String text, char align, String style, boolean textIsHTML, boolean sorted, String description,
                ...
                if (description != null && !description.equals("")) {
                    ...
                    client.registerTooltip(VMagnoliaTable.this, td, info);
                } else {
                    // Remove possibly previously set tooltip
                    client.registerTooltip(VMagnoliaTable.this, td, null);
                }
            ...
            private boolean handleClickEvent(Event event, Element targetTdOrTr, boolean immediate) {
                if (!client.hasEventListeners(VMagnoliaTable.this, ITEM_CLICK_EVENT_ID)) {
            ...
            private void handleTooltips(final Event event, Element target) {
                if (target.hasTagName("TD")) {
                    ...
                    if (!containsWidget) {
                        ...
                        if (client.getTooltipTitleInfo(VMagnoliaTable.this, target) != null) {
                            // Cell has description, use it
                            client.handleTooltipEvent(event, VMagnoliaTable.this, target);
                        } else {
                            // Cell might have row description, use row
                            // description
                            client.handleTooltipEvent(event, VMagnoliaTable.this, target.getParentElement());
                        }
                    }
            ...
            public void onBrowserEvent(final Event event) {
                ...
                if (enabled && !touchEventHandled) {
                    ...
                    if (type == Event.ONCONTEXTMENU) {
                        ...
                        if (enabled && (actionKeys != null || client.hasEventListeners(VMagnoliaTable.this, ITEM_CLICK_EVENT_ID))) {
                            /*
             ...
             * 
             * 
             * NOTE: checkbox selection toggling added by fork.
             */
            public void toggleSelection() {
                ...
                if (selected) {
                    ...
                    selectionCheckBox.setValue(true, false);
                } else {
                    removeStyleName("v-selected");
                    selectionCheckBox.setValue(false, false);
                    selectedRowKeys.remove(String.valueOf(rowKey));
                }
                selectAll.setValue(selectedRowKeys.size() == scrollBody.renderedRows.size(), false);
            }
    ...
    private final Timer lazyAdjustColumnWidths = new Timer() {
        ...
        public void run() {
            ...
            if ((height == null || "".equals(height)) && totalRows == pageLength) {
                ...
                if (heightBefore != getOffsetHeight()) {
                    Util.notifyParentOfSizeChange(VMagnoliaTable.this, false);
                }
            }
            scrollBody.reLayoutComponents();
            Scheduler.get().scheduleDeferred(new Command() {
    ...
    private int getBorderWidth() {
        if (borderWidth < 0) {
            borderWidth = Util.measureHorizontalPaddingAndBorder(scrollBodyPanel.getElement(), 2);
            if (borderWidth < 0) {
                borderWidth = 0;
            }
        }
    ...
    private int getContentAreaBorderHeight() {
        if (contentAreaBorderHeight < 0) {
            if (BrowserInfo.get().isIE7() || BrowserInfo.get().isIE6()) {
                contentAreaBorderHeight = Util.measureVerticalBorder(scrollBodyPanel.getElement());
            } else {
    ...
    protected String buildCaptionHtmlSnippet(UIDL uidl) {
        ...
        if (uidl.hasAttribute("icon")) {
            s = "<img src=\"" + Util.escapeAttribute(client.translateVaadinUri(uidl.getStringAttribute("icon"))) + "\" alt=\"icon\" class=\"v-icon\">" + s;
        }
        return s;
    }
    ...
    private static class TableDDDetails {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof TableDDDetails) {
                ...
                return dropLocation == other.dropLocation && overkey == other.overkey
    ...
    private int getFullyVisibleRowCount() {
        return (int) (scrollBodyPanel.getOffsetHeight() / scrollBody.getRowHeight());
    }
