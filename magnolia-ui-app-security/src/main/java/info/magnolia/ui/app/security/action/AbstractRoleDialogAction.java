...
public abstract class AbstractRoleDialogAction<D extends ActionDefinition> extends AbstractAction<D> {
    ...
    protected DialogDefinition getDialogDefinition(String dialogName) throws ActionExecutionException {
        ...
        try {
            ...
            Node node = MgnlContext.getJCRSession(RepositoryConstants.CONFIG).getNode("/modules/ui-security-app/dialogs/" + dialogName);
            ...
            if (!hasTab(dialogDefinition.getForm(), "acls")) {
                Node tabNode = MgnlContext.getJCRSession(RepositoryConstants.CONFIG).getNode("/modules/ui-security-app/dialogs/roleAdd/form/tabs/acls");
            ...
            if (!hasTab(dialogDefinition.getForm(), "web")) {
                Node tabNode = MgnlContext.getJCRSession(RepositoryConstants.CONFIG).getNode("/modules/ui-security-app/dialogs/roleAdd/form/tabs/web");
                dialogDefinition.getForm().getTabs().add((TabDefinition) Components.getComponent(Node2BeanProcessor.class).toBean(tabNode, ConfiguredTabDefinition.class));
            }
