...
public class WorkbenchViewImpl extends VerticalLayout implements WorkbenchView, Serializable {
    ...
    @Override
    public void addContentView(String viewType, ContentView view, ContentPresenterDefinition contentViewDefinition) {
        ...
        if (!contentViewDefinition.isShowButton()) {
            return;
        }

        // set button
        Button button = buildButton(viewType, contentViewDefinition.getIcon(), contentViewDefinition.isActive());
        contentViewsButton.put(viewType, button);
        viewModes.addComponent(button);
        // set active
        if (contentViewDefinition.isActive()) {
            currentViewType = previousViewType = viewType;
        }
    }

    @Override
    public Map<String, ContentView> getContentViews() {
        return contentViews;
    }
