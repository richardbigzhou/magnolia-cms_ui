...
public class WorkspaceAccessControlList extends AccessControlList<WorkspaceAccessControlList.Entry> {
    ...
    Collection<Entry> mergeEntries(Collection<Entry> entries) {
        ...
        Map<Entry.Key, Collection<Entry>> entriesGroupedByKey = Multimaps.index(entries, new Function<Entry, Entry.Key>() {
            @Override
            public Entry.Key apply(Entry entry) {
                return entry.getKey();
            }
        }).asMap();

        // Drop the keys and merge entries within same groups
        Set<Entry> mergedEntries = new LinkedHashSet<>();
        for (Collection<Entry> mergeableEntries : entriesGroupedByKey.values()) {
            Iterator<Entry> it = mergeableEntries.iterator();
            // assuming multimaps won't create empty entries
            Entry mergedEntry = it.next();
            while (it.hasNext()) {
                mergedEntry = mergedEntry.merge(it.next());
            }
