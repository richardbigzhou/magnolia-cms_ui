...
public class DuplicateUserActionTest extends RepositoryTestCase {
    ...
    public void setUp() throws Exception {
        ...
        userManager.setRealmName(Realm.REALM_ADMIN.getName()); // avoid exception in MgnlUserManager when looking for node at /{realmName}
    ...
    @Test
    public void testUserACLPropertyPathChangeWhileDuplicateNode() throws Exception {
        // GIVEN
        ...
        final DuplicateUserAction action = new DuplicateUserAction(new DuplicateUserActionDefinition(), userItem, mock(EventBus.class));

        // WHEN
        action.execute();

        // THEN
        assertTrue(adminNode.hasNode("testUser0"));
        assertEquals("/admin/testUser0", adminNode.getNode(user.getName() + "0" + "/acl_users/0").getProperty("path").getString());
    }
}
