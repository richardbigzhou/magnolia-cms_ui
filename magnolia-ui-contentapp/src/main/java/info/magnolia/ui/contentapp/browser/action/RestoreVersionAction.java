...
public class RestoreVersionAction extends ShowVersionsAction<RestoreVersionActionDefinition> {
    ...
    /**
     * @deprecated since 5.4.8 - use {@link #RestoreVersionAction(RestoreVersionActionDefinition, AppContext, LocationController, UiContext, FormDialogPresenter, AbstractJcrNodeAdapter, SimpleTranslator, VersionManager, EventBus, VersionConfig, ContentConnector, Context)}
     */
    @Deprecated
    public RestoreVersionAction(RestoreVersionActionDefinition definition, AppContext appContext, LocationController locationController, UiContext uiContext, FormDialogPresenter formDialogPresenter, AbstractJcrNodeAdapter nodeAdapter, SimpleTranslator i18n, VersionManager versionManager, final @Named(AdmincentralEventBus.NAME) EventBus eventBus, VersionConfig versionConfig, ContentConnector contentConnector) {
        this(definition, appContext, locationController, uiContext, formDialogPresenter, nodeAdapter, i18n, versionManager, eventBus, versionConfig, contentConnector, Components.getComponent(Context.class));
    }
    ...
    protected Version createVersionBeforeRestore(Node node) throws RepositoryException {
        VersionCommand versionCommand = new VersionCommand(versionManager);
