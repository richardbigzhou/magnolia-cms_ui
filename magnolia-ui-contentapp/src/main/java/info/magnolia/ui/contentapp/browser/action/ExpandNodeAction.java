...
public class ExpandNodeAction extends AbstractAction<ExpandNodeActionDefinition> {
    ...
    @Inject
    public ExpandNodeAction(ExpandNodeActionDefinition definition, JcrItemAdapter item, @Named(AdmincentralEventBus.NAME) EventBus admincentralEventBus) {
        super(definition);
        this.item = item;
    ...
    public void execute() throws ActionExecutionException {
        if (item.getJcrItem().isNode()) {
            admincentralEventBus.fireEvent(new ContentChangedEvent(item.getItemId(), true));
        }
    }
}
